...
def start():
    ...
    while not rospy.is_shutdown():
        ...
        if lines2 is not None and lines is not None:
            # roi1_pt2(왼쪽 영역 위쪽 x좌표) 값과 roi2_pt1(오른쪽 영역 위쪽 x좌표) 값을 평균내어 두 라인의 중간값을 x_pt에 저장함.
            x_pt = (lines2[0][0][0] + lines[0][0][2]) / 2 - 320
            # 두 선 검출 y좌표는 카메라 이미지 세로 480중 180으로 설정함.
            y = 180
            # 차량이 바라보는 정면과 라인사이 중간값의 차이를 atan2함수에 y, x_pt를 대입해서 구하고, 차량의 각도(angle)을 atan2함수값으로 설정.
            angle = atan2(y, x_pt)
            # 직선 주행시 속도는 최고속도 50으로 설정.
            speed = 50
            # 차량이 바라보는 정면이 라인사이 중간값보다 작은 경우 angle은 음수이므로 (-) 설정.
            if x_pt <= 0:
                angle = -angle
        ...
        # 왼쪽 라인만 검출 되는 경우
            ...
            lines2 = tmp2[-2]
            # roi1_pt2(왼쪽 영역 위쪽 x좌표) 값과 차량 카메라 정면 값(320)의 차이를 x_pt에 저장.
            x_pt = 320 - lines[0][0][2]
            # 두 선 검출 y좌표는 카메라 이미지 세로 480중 60으로 설정함.
            y = 60
            # 차량이 바라보는 정면과 라인사이 중간값의 차이를 atan2함수에 y, x_pt를 대입해서 구하고, 차량의 각도(angle)를 atan2함수값에 가중치(22)를 곱함.
            angle = 22 * atan2(y, x_pt)
            # 커브구간 주행시 속도는 42로 설정.
            speed = 42
            # 차량이 바라보는 정면이 라인사이 중간값보다 작은 경우 angle은 음수이므로 (-) 설정.
            if x_pt <= 0:
        ...
        # 오른쪽 라인만 검출 되는 경우
            ...
            lines = tmp1[-2]
            # roi2_pt1(오른쪽 영역 위쪽 x좌표) 값과 차량 카메라 정면 값(320)의 차이를 x_pt에 저장.
            ...
            if x_pt <= 0:
                angle = -angle
        # 어떤 라인도 검출되지 않는 경우, 현재 주행방향 유지, 속도 40으로 설정.
        else:
            angle = 0
            speed = 40
